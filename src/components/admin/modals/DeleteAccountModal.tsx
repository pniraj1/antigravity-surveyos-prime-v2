    ...
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4" style={{ background: 'rgba(13,27,42,0.85)', backdropFilter: 'blur(4px)' }}>
      <div className="w-full max-w-md rounded-2xl shadow-2xl overflow-hidden" style={{ background: '#FFFFFF', border: '1px solid #E2E6EA' }}>
        <div className="px-6 py-5 border-b border-[#F0F2F5] flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-black text-[#0D1B2A]">Delete Account</h3>
            <p className="text-[11px] text-[#8D99AE] font-semibold">{surveyor.name} — {surveyor.email}</p>
          </div>
        </div>
        <div className="px-6 py-5 space-y-4">
          <div className="p-4 rounded-xl border border-red-200 bg-red-50 flex items-start gap-3">
            ...
            <div className="space-y-1">
              <p className="text-xs font-black text-red-800">This action is irreversible.</p>
          ...
          <div>
            <label className="block text-[10px] font-black uppercase tracking-wider text-[#8D99AE] mb-2">
              Type <span className="text-red-600 font-mono">{surveyor.name}</span> to confirm
            </label>
            <input
              type="text"
              ...
              style={{ background: '#F8F9FA', border: `1px solid ${nameMatches ? '#A7F3D0' : '#E2E6EA'}`, color: '#0D1B2A' }}
            />
          </div>
        </div>
        <div className="px-6 py-4 border-t border-[#F0F2F5] flex gap-3 justify-end">
          <button
            onClick={onCancel}
            disabled={processing}
            className="px-4 py-2 rounded-xl text-xs font-bold text-[#8D99AE] hover:bg-[#F0F2F5] transition-all disabled:opacity-50"
          >
            Cancel
          </button>
          <button
            ...
            className="flex items-center gap-2 px-5 py-2 rounded-xl text-xs font-black bg-red-600 text-white hover:bg-red-700 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
          >
            <Trash2 size={12} />
